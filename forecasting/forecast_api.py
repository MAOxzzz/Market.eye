...
class ForecastAPI:
    ...
    def get_available_tickers(self):
        ...
        # Return all unique tickers from the dataset
        try:
            df = pd.read_csv(os.path.join(self.data_dir, 'Dataset.csv'))
            if 'Ticker' in df.columns:
                return sorted(df['Ticker'].unique().tolist())
            else:
                # Fallback to defaults if Ticker column is missing
    ...
    def get_historical_and_forecast(self, ticker, days=30, historical_days=90):
        ...
        try:
            ...
            if len(df) < 30:
                print(f"Not enough data points for {ticker}. Found {len(df)} rows.")
                return None, None
    ...
    def get_forecast(self, ticker, days=30):
        ...
        """
        _, forecast_df = self.get_historical_and_forecast(ticker, days)
        return forecast_df
